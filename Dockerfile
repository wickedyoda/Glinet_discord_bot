...
we = os.getenv('WEB_ENABLED', 'true').strip().lower() not in {'0', 'false', 'no', 'off'}; \
sys.exit(0) if not we else None; \
port = int(os.getenv('WEB_PORT', '8080') or '8080'); \
r = urllib.request.urlopen(f'http://127.0.0.1:{port}/readyz', timeout=8); \
