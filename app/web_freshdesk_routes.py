...
def viewer_page():
    ...
    if request.method == "POST":
        ...
        if callable(on_save_env):
            ...
            admin_role_id = str(request.form.get("FRESHDESK_ADMIN_ROLE_ID", "0")).strip()
            user_role_id = str(request.form.get("FRESHDESK_USER_ROLE_ID", "0")).strip()

            updates = {
                "FRESHDESK_ENABLED": str(enabled).lower(),
                "FRESHDESK_BASE_URL": base_url,
                "FRESHDESK_REQUEST_TIMEOUT_SECONDS": timeout,
                "FRESHDESK_ADMIN_ROLE_ID": admin_role_id,
                "FRESHDESK_USER_ROLE_ID": user_role_id,
            }
